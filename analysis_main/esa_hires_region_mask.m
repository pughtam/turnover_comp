function [mask_broad_ever,mask_broad_dec,mask_needl_ever,mask_needl_dec,...
    mask_tree_mixed,mask_tree_shrub_mosaic,mask_tree_flooded,esa_05]=esa_hires_region_mask()
...
mask_broad_ever=false(size(esa_05));
mask_broad_ever(esa_05==50)=true; % 50;Tree cover, broadleaved, evergreen, closed to open (>15%);0;100;0
mask_broad_dec=false(size(esa_05));
mask_broad_dec(esa_05==60 | esa_05==61 | esa_05==62)=true; % 60;Tree cover, broadleaved, deciduous, closed to open (>15%);0;160;0 % 61;Tree cover, broadleaved, deciduous, closed (>40%);0;160;0 % 62;Tree cover, broadleaved, deciduous, open (15-40%);170;200;0
mask_needl_ever=false(size(esa_05));
mask_needl_ever(esa_05==70 | esa_05==71 | esa_05==72)=true; % 70;Tree cover, needleleaved, evergreen, closed to open (>15%);0;60;0 % 71;Tree cover, needleleaved, evergreen, closed (>40%);0;60;0 % 72;Tree cover, needleleaved, evergreen, open (15-40%);0;80;0
mask_needl_dec=false(size(esa_05));
mask_needl_dec(esa_05==80 | esa_05==81 | esa_05==82)=true; % 80;Tree cover, needleleaved, deciduous, closed to open (>15%);40;80;0 % 81;Tree cover, needleleaved, deciduous, closed (>40%);40;80;0 % 82;Tree cover, needleleaved, deciduous, open (15-40%);40;100;0
mask_tree_mixed=false(size(esa_05));
mask_tree_mixed(esa_05==90)=true; % 90;Tree cover, mixed leaf type (broadleaved and needleleaved);120;130;0
mask_tree_shrub_mosaic=false(size(esa_05));
mask_tree_shrub_mosaic(esa_05==100)=true; % 100;Mosaic tree and shrub (>50%) / herbaceous cover (<50%);140;160;0
mask_tree_flooded=false(size(esa_05));
mask_tree_flooded(esa_05==160 | esa_05==170)=true; % 160;Tree cover, flooded, fresh or brakish water;0;120;90 % 170;Tree cover, flooded, saline water;0;150;120

save esa_hires_region_mask.mat mask_broad_ever mask_broad_dec mask_needl_ever...
    mask_needl_dec mask_tree_mixed mask_tree_shrub_mosaic mask_tree_flooded esa_05
